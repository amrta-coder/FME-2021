...
def extract_featrues_fixed_len(df, onset_offset_frame_list, openface_featrue_file_path, len_frame=8, step=4):
    ...
    openface_featrue_file_save = os.path.join(features_engineered_root, path_list[-2], path_list[-1].split('.')[0])
...
def featrue_engineer_openface(openface_featrue_file_path, df_label_org, label_all):
    ...
    label_list = extract_featrues_fixed_len(df_openface_AU_r, onset_offset_frame_list, openface_featrue_file_path)
...
def main():
    ...
    df_label[['file_path', 'label']].to_csv(os.path.join(features_engineered_root, label_casme2_predict), index=False)
