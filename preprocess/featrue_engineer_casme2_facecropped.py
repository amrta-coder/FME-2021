...
def featrue_engineer_openface(openface_featrue_file_path, df_label_org, label_all):
    ...
    mmsc = MinMaxScaler()
    df_openface_AU_r = mmsc.fit_transform(df_openface_AU_r)
    dump(mmsc, open('MinMax_scaler.pkl', 'wb'))
...
def main():
    ...
    df_label.to_csv(os.path.join(features_engineered_root, label_croped_file_vad), index=False)
