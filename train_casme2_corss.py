...
def main(output_dir, optimizer):
    ...
    for subject in list_subject:
        ...
        df_label['subject'] = df_label['file_path'].map(lambda x: x.split('/')[-1][:2])
        df_label_train = df_label.loc[(df_label['subject'] != str(subject))]
        df_label_val = df_label.loc[(df_label['subject'] == str(subject))]
