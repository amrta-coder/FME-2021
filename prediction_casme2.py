...
def main():
    ...
    model_path = os.path.join(output_dir_casme2, output_model_name)
    fme_dir = cp["TRAIN"].get("fme_dir")
    features_engineered = cp["DEFAULT"].get("features_engineered")
    label_casme2_predict = cp["TEST"].get("label_casme2_org_predict")
    ...
    n_gru_units = cp["TEST"].getint("n_gru_units")
    test_csv = os.path.join(fme_dir, features_engineered, label_casme2_predict)
    ...
    df_test.to_csv(os.path.join(output_dir_casme2, 'result.csv'), index=False)
    evaluate_file = os.path.join(output_dir_casme2, 'threshold.log')
