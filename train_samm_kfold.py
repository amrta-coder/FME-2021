...
def save_config(output):
    ...
    output = os.path.join(output, str(datetime.now().strftime("%Y%m%d-%H%M%S")+'_samm_'+model_name+str(n_gru_units)))
...
def main(output_dir, optimizer):
    ...
    for i, (train_index, test_index) in enumerate(skf.split(X, Y)):
        ...
        if model_name == 'ConcatCNN_SAMM':
            model = model_method(n_timesteps=n_timesteps, n_features=n_features)
        else:
            model = model_method(n_timesteps=n_timesteps, n_features=n_features, n_gru_units=n_gru_units, n_dense_units=n_dense_units)
        print(model.summary())
...
if __name__ == "__main__":
    ...
    n_dense_units = cp["TRAIN"].getint("n_dense_units")
    ...
    df_label = pd.read_csv(os.path.join(features_engineered_root, label_samm))
